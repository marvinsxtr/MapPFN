...
def process_rna_data(cache_path: str = "datasets/single_cell", seed: int = 42) -> None:
    ...
    train_adata.obs[ColumnNames.TREATMENT] = train_adata.obs[ColumnNames.TREATMENT].cat.remove_unused_categories()

    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        sc.tl.rank_genes_groups(
            train_adata, groupby=ColumnNames.TREATMENT, reference=Values.CONTROL, n_genes=50, rankby_abs=True
        )
